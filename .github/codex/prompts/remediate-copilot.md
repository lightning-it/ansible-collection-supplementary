...
Remediate all unresolved GitHub Copilot review findings that apply to the exact
head SHA supplied in `CODEX_EXPECTED_HEAD_SHA` as one bounded correction package.
Use `gh api graphql` once to retrieve the complete thread set for
`CODEX_PR_NUMBER` in `CODEX_REPOSITORY`; accept comments only from
`copilot-pull-request-reviewer[bot]`. Never create one edit cycle per thread.
Re-read the remote PR head before editing and again before finishing. Stop
without editing if it differs from the expected SHA.
...
the repository's corresponding deterministic gate already passes on the exact
head. Document that evidence in one reply instead. Do not manufacture a no-op
commit. After one material correction package, only one final Current-Head
Copilot re-review is allowed; new material findings in that final review stop
fail-closed instead of starting another Codex/Copilot loop.
