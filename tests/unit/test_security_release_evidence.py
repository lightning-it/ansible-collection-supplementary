...
class ProducerEvidenceTests(unittest.TestCase):
    def test_evidence_binds_every_release_asset(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cmd = ["python3", str(SCRIPT), "--id", "LIT-SEC-TEST", "--security-id", "LIT-SEC-TEST", "--affected-version", "3.1.0", "--version", "3.1.2", "--source-sha", "1" * 40, "--workflow-ref", "2" * 40, "--consumer", "lightning-it/container-ee-wunder-ansible-ubi9", "--acceptance-profile", "lit.supplementary/test", "--created-at", "2026-07-31T00:00:00Z", "--not-before", "2026-07-31T00:00:00Z", "--expires-at", "2026-08-07T00:00:00Z", "--output", str(output)]
            for name, path in files.items(): cmd += [f"--{name}", str(path), f"--{name}-url", f"https://example.invalid/{name}"]
            ...
            for name in ("signature", "sbom", "provenance"):
                self.assertEqual(value["artifact"][name]["digest"], "sha256:" + hashlib.sha256(name.encode()).hexdigest())
