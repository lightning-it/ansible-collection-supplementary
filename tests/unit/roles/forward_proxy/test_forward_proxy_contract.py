...
class ForwardProxyContractTests(unittest.TestCase):
    ...
    def test_disabled_state_cleans_only_role_owned_state(self) -> None:
        ...
        self.assertIn("lit.supplementary.forward_proxy.managed-state/v2", tasks)
    ...
    def test_check_mode_still_checks_image_without_waiting_for_listener(self) -> None:
        tasks = (ROLE_ROOT / "tasks" / "enabled.yml").read_text()
        image_task, wait_task = tasks.split("- name: Wait for the local Squid listener", maxsplit=1)
