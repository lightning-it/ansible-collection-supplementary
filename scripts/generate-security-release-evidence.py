...
def main():
    ...
    p.add_argument("--created-at", required=True); p.add_argument("--not-before", required=True); p.add_argument("--expires-at", required=True)
    ...
    try:
        ...
        p.error(str(exc))
    if expires <= not_before: p.error("--expires-at must be after --not-before")
    evidence = {
        ...
        "artifact": {"collection": "lit.supplementary", "version": a.version, "digest": sha256(a.artifact), "releaseUrl": a.artifact_url, "signature": file_ref(a.signature, a.signature_url), "sbom": file_ref(a.sbom, a.sbom_url), "provenance": file_ref(a.provenance, a.provenance_url)},
