...
- Add a descriptor-relative atomic_path module for identity-bound directory creation and regular-file replacement without following parent symlinks.
- Add the distribution-neutral lit.supplementary.forward_proxy role for a closed, digest-pinned, hardened Squid Podman service on Ubuntu and Enterprise Linux hosts.
- guacamole_deploy - add a bounded container API inactivity timeout and run role prechecks before any host mutation.

Bugfixes
--------

- Bind forward-proxy rollback handlers to the explicit first mutation boundary after read-only preflight, including failures that occur inside a mutating operation.
- Bind main-promotion review to the exact protected controller and invoke the verifier helper through mutually exclusive exact-revision reusable calls, with a fail-closed maintained pin that must advance through protected develop before each later main promotion.
- Compare the checked-out synthetic integration tree when resolving a detached release head.
- Correct the managed-file rollback path and preserve per-contract Tiny evidence with profile-derived release eligibility.
- Enforce immutable merged-PR identity for release and back-sync push changelog authorization.
- Fail closed before removing ownership evidence when a managed runtime loses its Quadlet.
- Keep disabled unowned forward proxy invocations mutation-free, bind Squid readiness to protocol evidence and the revalidated runtime, and reject boolean Guacamole host ports.
- Pin the managed rollback path to decode slurped Squid policy content exactly once.
- Preserve a fail-closed Tiny JUnit artifact even when forward-proxy verification aborts before the final per-contract report is assembled, without publishing successful test counters for an incomplete run.
- Preserve exact release authorization across paginated PR associations and the offline devtools boundary.
- Preserve fail-closed forward-proxy Tiny JUnit evidence when convergence fails before verification starts.
- Push-Ready now canonicalizes its private check-state path so protected Exact-Revision tests also work on hosts with symlinked temporary roots.
- Push-Ready now provides a private per-run RUNNER_TEMP so protected Exact-Revision materializer tests can execute inside the hermetic check environment.
- Reject YAML booleans at the forward proxy lock, readiness, and upstream port integer boundaries so malformed inputs fail closed.
- Reject YAML booleans for the forward proxy listen port, report owned disable transitions in check mode, and require same-repository Release App identity and the canonical base branch before a release-named pull request receives changelog privileges. Generated changelog authorization now runs before fragment-exemption labels, protocol readiness uses Ansible's configured-or-discovered Python interpreter and complete HTTP headers, and connection validation remains redacted. Explicitly configured target Python interpreters remain supported, while release, back-sync, and develop-to-main promotion privileges are bound to canonical repositories and target branches on both pull-request and post-merge push checks; manual and unknown event types fail closed.
- Reject crafted single-parent release merge subjects and numeric proxy identities outside the atomic writer range.
- Report planned nested forward-proxy directories as changed in check mode.
- Report render-only runtime removal, activation, and restart operations in check mode, and persist a resumable runtime-absent checkpoint before deleting owned forward proxy files.
- Restrict detached release and backsync ref recovery to canonical stable semantic-version branch names.
- Validate non-empty atomic-unlink checksums before the absent-target fast path.
- Verify the pinned forward-proxy image before any enabled-state file or runtime mutation so a missing preload leaves existing state untouched.

New Modules
-----------

- lit.supplementary.atomic_path - Mutate one path below an identity\-bound parent chain.
- lit.supplementary.atomic_unlink - Unlink one exactly identified regular file.
...
- Add Vault-only secret bundle orchestration for application services.
- Add a protected, no-AI REP-60 bootstrap verifier that can publish the legacy review alias only for the exact controller introduction and its normal develop-to-main promotion.
- Add an explicit NGINX switch for profiles that must reject local certificate files and enforce Vault-only TLS custody.
- Add base-policy-bound standard and trust-root review profiles, exact-head finalization, parallel dual review, and a single bounded remediation round.
- Add digest-pinned NetBox and Apache Guacamole Podman deployment roles.
...
- Add the bounded MLX-90 push-ready foundation. It validates the exact current-head patch, rejects unsafe review inputs, and records only non-secret integrity evidence before a normal protected-branch push. Its temporary working directory also keeps nested container-wrapper probes independent from linked-worktree Git metadata. The initial foundation uses a review-only, all-or-nothing trust-root bootstrap; validate and push-ready evidence become available only after the protected foundation merge.
- Add the inert, versioned MLX-90 Security release contract library and its fail-closed unit contract before activating any production consumer.
- Bind the human Current-Revision producer's protected-verifier handoff to the exact develop or main base ref, base SHA, head SHA, PR number, and producer run so the helper receives its complete fail-closed input contract.
- Complete the REP-60 review-gate cutover by removing the immutable bootstrap controllers and the obsolete Successful Copilot review alias; permanent producers now publish only the protected Current revision review result, and the rerun helper verifies the required workflow against the live candidate ref and SHA GitHub actually reports.
- Give optional protected-handoff inputs explicit empty defaults and inventory every declared Galaxy dependency before an offline local gate runs. Keep the default artifacts-only scenario explicitly dependency-free, while any other selected Molecule scenario fails immediately when the pinned image lacks a declared dependency.
- Make main-to-develop ancestry backmerges permanently reviewable under the MLX-90 section 7.2 empty-input rule by adding a deterministic protected main binding as the merge commit's only develop-tree change.
- Route evidence-bound MLX-90 Security releases through an exact-byte native Nexus Galaxy v3 stage and signed ModuLix Heavy/Application validation receipt before publishing the unchanged candidate to Ansible Galaxy.
- Route only exhaustively verified Release-App main-to-develop ancestry backmerges through the deterministic REP-60 zero-AI current-revision exception while retaining MLX-90 section 7.2 Codex review for every other Release-App pull request.
- Run local unmanaged Molecule scenarios in the pinned Devtools container without network or a mounted Docker socket, and fail closed when a scenario requires a managed runtime that belongs to the protected pipeline. Bind every scenario's Molecule ephemeral directory explicitly to container tmpfs rather than the read-only workspace.
- Split the accumulated REP-60 main promotion so every protected review stays below the immutable 200000-byte input ceiling without weakening the limit. The first stage retains the bounded-diff materializer, remediation guards, prompt, verifier tests, and recoverable check reservation while deferring only documentation and changelog metadata to the second promotion.
- Update the immutable Samba AD-LDS container image digest and its canonical source-dependency inventory entry to the current approved image artifact.
- mlx90 - Add the protected Exact-Revision Codex exception for finalized Release-App pull requests, with immutable base, head, merge-base, integration-tree, full binary-diff, protected-review-asset, and idempotent complete-input bindings; bind the neutral result to a protected required workflow and the exact producer run instead of trusting its status name or caller-supplied check URL; local Push-Ready remains AI-free.
- mlx90 - Run required history-free Codex and Copilot push-ready reviews in parallel, bind their evidence to distinct workspaces and identical inputs, and reject review payloads at the exclusive size boundary before expensive checks.

Security Fixes
--------------

- Add a pinned Base verifier for reviewed Trust-Root engine updates so the candidate engine cannot validate its own Base-bound evidence. The verifier must already exist unchanged in the pinned Base and cannot bootstrap or approve itself, invoke local external reviewers, or replace shared Git hooks. Repository discovery and every Git read use an isolated credential-free environment, while non-regular candidate engines fail closed before Base code is loaded.
- Add the stable Main promotion merge gate required context. It always materializes for pull requests to main and succeeds only after both exact promotion classification and the selected protected environment authorization succeed, while ordinary promotions retain their human approval environment.
- Allow the Release Automation App to refresh the one-time recovery receipt branch onto a newer protected-main base only after revalidating the prior App commit, its complete receipt contract, and an exact force-with-lease.
- Bind MLX-90 Security intake and release preparation to duplicate-safe canonical chain identifiers, immutable receipts, and exact automation identities.
- Bind the observed Chrome for Linux 151.0.7922.108 runtime to the reviewed paired 151.0.7922.109 platform-neutral scanner CPE while preserving the observed package URL, binary digest, and source evidence.
- Enforce independent Grype and Trivy MLX-90 Security release scans without VEX, ignore, or allowlist exceptions so every High or Critical finding remains release-blocking.
- Keep the protected main-promotion workflow compatible while the immutable event base still has the preceding verifier interface. The compatibility path accepts only the exact normal develop-to-main promotion and keeps release and Security heads fail-closed until content verification is available from main.
- Make the MLX-90 Security-release classifier an explicit Security & Compliance CODEOWNERS trust root so its reviewed governance digest cannot be changed without the protected ownership path.
- Pre-register the fail-closed, immutable acceptance profile and offline verifier for the Keycloak 26.7.1 CVE remediation before any release candidate can claim that profile, and allow only the exact canonical source-dependency inventory path required by an image Security update.
- Recognize the one-time App-only Security recovery receipt in collection, changelog, and protected-main promotion gates only after revalidating its exact historical candidate topology, canonical diff, paths, and immutable metadata and changelog bindings.
- Remove the Galaxy-first transition no-op and fail closed on missing Nexus configuration, foreign GitHub App/run identity, ambiguous validation runs, unsigned or substituted receipts, and Nexus or Galaxy readback mismatch.
- Replace the 20 centrally classified committed credential-shaped test values with runtime-generated, ephemeral credentials, protected task output, restrictive lookup-file permissions, and explicit before/after redaction assertions. Documentation now lists only required secret variable names and keeps their values in an encrypted Vault or approved external secret manager. Secret-bearing Grafana and Checkmk Pod manifests are now written as `root:root` with mode `0600` and covered by a regression contract. The temporary centrally managed manifest intentionally remains unchanged for this cleanup PR: the review engine reads it from the authoritative base and masks only exact old path, line, and content matches. Shared Assets retires that manifest through its guarded synchronizer only after this cleanup is merged, so replacement or newly introduced lines remain fully scanned.
- Require an explicit false Security classification for normal release execution and an explicit true classification plus the release App identity for MLX-90 Zero-Touch execution. Empty or malformed classifier output now stops before protected environments or release credentials are selected.
- Route the protected-main Security controller to the intake through an exact App-authenticated workflow dispatch with live installation, allowlist, permission, commit, pull-request, and native merge proof.
- Stream paginated Current-Head check evidence into the trusted Shared Assets auto-merge guard instead of passing it through process arguments, so large protected matrices remain fail-closed without exceeding the host ARG_MAX.
...
- Accept an existing App-authored Security PR's immutable GitHub base snapshot only when it is a verified ancestor of the exact protected main revision, allowing the recovery receipt branch to refresh without weakening its current-head, author, topology, allowlist, or native auto-merge checks.
- Add a one-time fail-closed, release-App-only recovery for the immutable 3.2.4 Security marker that reached protected main without its intake receipt, including an immutable main-ref relay for environments restricted to protected main.
- Add a protected, narrowly scoped bootstrap controller so the REP-60 Current-Revision workflow can be introduced without candidate self-certification or a required-check bypass.
- Align protected normal-promotion authorization with the documented five-operator small-team roster and permit those operators to approve their own promotions.
- Align v6 current-revision evidence with the protected rerun consumer.
- Allow the complete fail-closed repository-quality profile up to 3600 seconds so its full pre-commit and supplemental rootless Molecule gates can finish without weakening or skipping validation.
- Bind Copilot review instructions to the exact repository agent contract so history-free review fails closed on instruction drift.
- Bind every repository-owned executable used by the repository-quality profile to the immutable check-policy trust root.
- Bind pull-request-target run metadata to the candidate head independently from the protected controller revision, add a one-time fail-closed release-App rearm controller for the REP-60 main bootstrap, deduplicate only the bound bootstrap producer result rather than its protected verifier, and keep native required-workflow checks separate from same-named producer evidence.
- Bind the final one-time Security receipt recovery controller to the exact protected main and develop bases, allowlisted paths, and canonical diff.
- Bind the one-time Security receipt recovery to the exact protected post-changelog main and develop bases without weakening the general Security release chain.
- Bind the one-time Security recovery continuation to the exact protected main and develop state after the Incus runtime-evidence correction.
- Construct synthetic JWT test values at runtime so the history-free MLX-90 review bootstrap does not classify committed test literals as credentials.
- Document the exact historical synthetic review fixture for safe removal.
- Guard the Vault deploy flavour while resolving the controller-local Terraform state fallback so an inventory-provided state path remains usable when no deploy flavour is defined.
- Harden MLX-90 push readiness with complete collection gates, rootless parity, pinned validators, reviewed pushes, bounded UTF-8 review input, safe containers, and strict local Git checks.
- Initialize Python for the trusted release-bot Copilot review without an unavailable pre-checkout dependency cache, so protected release and promotion pull requests can complete their exact-head review gate.
- Keep artifact staging, AAP TLS trust, Cloudflare WARP rendering, Vault bootstrap, and Vault Raft restore coverage executable in the capability-minimized rootless light Molecule gate while restricting numeric ownership to explicit, portable, canonical, trailing-slash-free, owner-only unprivileged local test roots whose configured ownership equals the controller's effective non-zero UID and GID, refusing non-local execution before path inspection and symlinked AAP trust targets, while preserving root-owned production defaults.
- Keep later minor-release changelog fragments intact while an immutable intake-bound Security patch release consumes and publication revalidates only its reviewed fragment.
- Publish the temporary REP-60 bootstrap review alias as an unskippable, protected native job for the exact develop-to-main promotion, without a caller-controlled custom check or an artificial rerun-attempt cap.
- Reject non-UTF-8 files in the sanitized review workspace instead of replacing undecodable bytes before secret scanning.
- Run Tiny CI for the bounded REP-60 v5-successor promotion.
- Serialize the protected current-revision verifier re-evaluation PR number as a workflow-dispatch string so GitHub accepts the fail-closed handoff after a successful Exact-Revision Codex review.
- Stop emitting release dependency inventories when a late Incus cleanup snapshot can no longer prove the immutable base-image fingerprint.
- Treat the deterministic REP-60 main-ancestry record as non-user-visible governance metadata so protected back-sync pull requests do not require a self-referential changelog fragment.
- Validate the structured Exact-Revision Codex result as the single JSON object required by its schema instead of indexing it as an array.
- Wait for the immutable protected verifier Actions run to become terminal after its success check is visible, preventing transient GitHub status propagation from blocking the independent REP-60 trust root.
- vault_config - forward explicit state-only module upgrade migrations to the Terragrunt runner while keeping target-side state as the source of truth.
- vault_config - forward strict Terragrunt import handling through the temporary runtime-variable boundary so failed state adoption stops apply.
